...
- **The endpoint name is a registry key, not the subdomain**: a random subdomain is
  ...
  name, fold it once in preprocessing and publish it as a job output that the wait
  step reads, so both sides use the exact registered name
  (`workflows/ollama/yamls/general.yaml`). Killing the client process (cancel
