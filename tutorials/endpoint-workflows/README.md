...
The thing we are automating lives in [`tutorials/demo-app/`](../demo-app/README.md): a small script that renders a Mandelbrot fractal *and* serves a web page showing the progress live.
  ...
  run:
    ...
    steps:
      - name: Render and Serve
        ...
        run: pw endpoints run --name fractal-${PW_RUN_SLUG} -- ./demo-app/run.sh
      ...
      - name: Render and Serve Detached
        run: |
          if command -v setsid >/dev/null 2>&1; then
            setsid pw endpoints run --name fractal-${PW_RUN_SLUG} -- ./demo-app/run.sh > run.${PW_JOB_ID}.out 2>&1 < /dev/null &
          else
            nohup pw endpoints run --name fractal-${PW_RUN_SLUG} -- ./demo-app/run.sh > run.${PW_JOB_ID}.out 2>&1 < /dev/null &
          fi
...
```yaml
      - name: Create Run Script
        run: |
          ...
          pw endpoints run --name ${endpoint_name} -- ${PWD}/demo-app/run.sh
          EOF
          chmod +x script.sh
  ...
  script_submitter:
    ...
    steps:
      - name: Script Submitter
        uses: github/parallelworks/workflows@canary    # run another workflow as a step
        early-cancel: any-job-failed
        with:
          $yaml: workflows/script_submitter/v3.6/general.yaml        # which workflow inside that repo
...
`uses: github/parallelworks/workflows@canary` runs *another workflow* as a step. `$yaml` selects which workflow file inside that repo to run (here `workflows/script_submitter/v3.6/general.yaml`), and the remaining `with:` keys are that subworkflow's inputs — the script's path (published by `install` through `$OUTPUTS`) and the form's scheduler settings.
  ...
  fractal_demo:
    ...
    steps:
      - name: Fractal Demo
        ...
        with:
          $yaml: tutorials/endpoint-workflows/04-subworkflow.yaml
