  ...
  script_submitter:
    ...
    steps:
      - name: Script Submitter
        ...
        with:
          $yaml: workflows/script_submitter/v3.6/hsp.yaml        # which subworkflow to run
...
`uses: github/parallelworks/workflows@canary` runs *another workflow* as a step. `$yaml` selects which workflow file inside that repo to run (here `workflows/script_submitter/v3.6/hsp.yaml`), and the remaining `with:` keys are that subworkflow's inputs. 
...
Pick your resource, choose the matching configuration, and the right directives are filled in for you — no need to memorize each site's `select=...`. The samples are lifted from [`workflows/script_submitter/v3.6/hsp.yaml`](../../workflows/script_submitter/v3.6/hsp.yaml); tweak the walltime, queue, or node counts in the form for a given run.

### Lessons from the script submitter (what it does for you)

You hand the subworkflow a script and a few flags; in return it runs all the machinery you would otherwise hand-write. Reading [`workflows/script_submitter/v3.6/hsp.yaml`](../../workflows/script_submitter/v3.6/hsp.yaml) is the best way to see *why* each piece exists — the highlights:
